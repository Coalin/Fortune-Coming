...
def generate_dynamic_label(stock_return, index_return, potential_drawdown, lookback_window=20):
    ...
    if potential_drawdown is not None:
        high_volatility = potential_drawdown <= -0.10  # 潜在回撤超过10%
    else:
    ...
    print(f"最终标签分布:")
    print(f"  正样本(1): {(labels == 1).mean():.2%}")
    print(f"  负样本(0): {(labels == 0).mean():.2%}")
    print(f"  排除(NaN): {np.isnan(labels).mean():.2%}")
...
def backtest_evaluation(results_df, pred_proba, true_return, index_return, top_pct=0.1, holding_period=14):
    ...
    market_volatility = benchmark_return.std() * np.sqrt(periods_per_year) if len(benchmark_return) > 1 else 0.07
...
def main():
    ...
    feature_cols += get_momentum_feature_cols()
